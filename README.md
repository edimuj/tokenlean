...
MCP tools include the core context reducers (`tl_symbols`, `tl_snippet`, `tl_run`, `tl_impact`, `tl_browse`, `tl_tail`, `tl_guard`, `tl_diff`), workflow routers (`tl_advise`, `tl_pack`), briefing tools (`tl_analyze`, `tl_related`, `tl_context`, `tl_structure`, `tl_entry`), and GitHub batch operations (`tl_gh_issue_add_sub`, `tl_gh_issue_close_batch`, `tl_gh_issue_label_batch`, `tl_gh_project_add_batch`, `tl_gh_issue_create_batch`). Structured JSON in/out — no bash command construction, no argument parsing, no stdout scraping.
