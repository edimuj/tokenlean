...
The `tl_gh_*` tools accept both their native `repo: "owner/repo"` + `issues` shape **and** the GitHub-MCP convention (split `owner` + bare `repo`, `issue_number`) — pass either and it just works, no guess-and-check.
